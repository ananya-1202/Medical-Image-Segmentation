...
#resizing the images and the mask for processing 
from tqdm import tqdm
from itertools import chain
train_ids = next(os.walk(TRAIN_PATH))[1]
test_ids = next(os.walk(TEST_PATH))[1]

X_train = np.zeros((len(train_ids), h,w,c), dtype=np.uint8)
Y_train = np.zeros((len(train_ids), h,w, 1), dtype=np.bool)
...
X_test = np.zeros((len(test_ids), h,w,c), dtype=np.uint8)
sizes_test = []
